   ...
   /* public function detalleProformaID($idproforma)
    {
        ...
        $consulta = "SELECT * FROM detalleproforma DC, producto PR, proforma C, usuario U  WHERE DC.idproforma = '$idproforma' AND DC.idproducto=PR.idproducto AND DC.idproforma=C.idproforma AND C.dni=U.dni";
        $resultado = mysqli_query($this->conectar(),$consulta);
    ...
    public function detalleProformaMesa($idmesa)
    {
        ...
        $consulta = "SELECT * FROM detalleproforma DC, producto PR, proforma C, usuario U  WHERE C.mesanum = '$idmesa' AND DC.idproducto=PR.idproducto AND DC.idproforma=C.idproforma AND C.dni=U.dni AND C.estadocomprobante=1";
        $resultado = mysqli_query($this->conectar(),$consulta);
        $num_registros=mysqli_num_rows($resultado);
